...
async function fetchInitialData(userId: string, scopeSlug: string) {
  const [
    scopeRes,
    itemsRes,
    typesRes,
    ...
    supabase
      .from('scopes')
      .select('*')
      .eq('slug', scopeSlug as SystemScopeType)
      .single(),
      supabase
      .from('todos')
      .select(`
...
export default async function ScopeLayout({
  ...
  const initialData = await fetchInitialData(session.user.id, scopeSlug);
