...
async function fetchInitialData(userId: string) {
  const [
    ...
    supabase
      .from('scopes')
      .select('*')
      .eq('name', 'brainstorm' as SystemScopeType)
      .single(),
      supabase
      .from('todos')
      .select(`
        *,
        scope:scopes!todos_scope_id_fkey!inner (id, name, color, icon),
        type:types!left(
          id, 
          name,
          color
...
export default async function BrainstormLayout({
  children,
}: {
